...
- Proyecto sin ningún `project_stage` con fechas (`start_date`/`end_date` nulos): no aparece en el join cuando el filtro está activo (correcto, no se puede evaluar solapamiento sin fechas) pero sigue apareciendo cuando el filtro Desde/Hasta está vacío, igual que hoy.
