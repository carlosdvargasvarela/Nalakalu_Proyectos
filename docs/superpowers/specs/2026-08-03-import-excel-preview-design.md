...
1. **Aceptar `.xlsx`/`.xls` además de `.csv`**, detectado por extensión del archivo subido. Se agrega la gema `roo` (lectura de xlsx/xls; CSV se sigue leyendo con la librería `csv` de siempre).
...
```ruby
# Gemfile
gem "roo" # lectura de .xlsx/.xls para carga masiva; CSV sigue usando la stdlib
```
...
- El número de fila reportado (`index + 2`) se mantiene igual para los tres formatos.
...
2. Si NO hay `@preview` ni `@results`: formulario de subida, ahora apunta a `preview_imports_path` y acepta `.csv,.xlsx,.xls`.
...
- `.xls` (formato viejo de Excel): `roo` lo soporta igual que `.xlsx`, mismo código de parseo (Roo abstrae el formato).
- Extensión de archivo no reconocida (ni `.csv` ni `.xlsx` ni `.xls`): se trata como si no se hubiera subido archivo, con mensaje "Formato no soportado, subí un .csv, .xlsx o .xls".
